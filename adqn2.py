...
class PriorityReplayBuffer():
    def __init__(self, maxSize = 100000):
        ...
        self.size = mp.value("i")

        self._oldObs = torch.empty(maxSize,84,84,4) #n*84*84*4
        self._obs = torch.empty(maxSize,84,84,4) #n*84*84*4
        self._done = torch.empty(maxSize) #n
        self._reward = torch.empty(maxSize) #n
        self._action = torch.empty(maxSize) #n
        self._weights = []
